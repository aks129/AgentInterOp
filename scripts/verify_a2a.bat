...
curl -i -sS -X POST "%BASE_URL%/api/bridge/demo/a2a" -H "Content-Type: application/json" --data "{\"jsonrpc\":\"2.0\",\"method\":\"tasks/cancel\",\"params\":{\"id\":\"x\"},\"id\":3}"
echo.

echo == TEST 2: STREAMING call (message/stream) ==
echo Testing SSE streaming without Content-Length...
curl -i -sS -N -X POST "%BASE_URL%/api/bridge/demo/a2a" -H "Content-Type: application/json" -H "Accept: text/event-stream" --data "{\"jsonrpc\":\"2.0\",\"method\":\"message/stream\",\"params\":{\"message\":{\"parts\":[{\"kind\":\"text\",\"text\":\"test\"}]}},\"id\":1}"
echo.

echo == TEST 3: Context endpoint JSON ==
echo Testing context-aware endpoint...
curl -i -sS -X POST "%BASE_URL%/api/bridge/custom/a2a" -H "Content-Type: application/json" --data "{\"jsonrpc\":\"2.0\",\"method\":\"tasks/get\",\"params\":{\"id\":\"test\"},\"id\":5}"
echo.

echo == TEST 4: Context endpoint streaming ==
echo Testing context-aware endpoint with streaming...
curl -i -sS -N -X POST "%BASE_URL%/api/bridge/custom/a2a" -H "Content-Type: application/json" -H "Accept: text/event-stream" --data "{\"jsonrpc\":\"2.0\",\"method\":\"message/stream\",\"params\":{},\"id\":7}"
echo.

echo == TEST 5: Empty body handling ==
echo Testing error handling for empty body...
curl -i -sS -X POST "%BASE_URL%/api/bridge/demo/a2a" -H "Content-Type: application/json" --data ""
echo.
